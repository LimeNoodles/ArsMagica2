...
//		List<ItemStack> oredict = OreDictionary.getOres(stack.getItem().getUnlocalizedName());
		...
		if (stack.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
			//todo stack.getItem().getSubItems(stack.getItem(), stack.getItem().getCreativeTab(), alternates);
		}
		...
		if (isEssenceRefiner){
			...
			if (recipe != null){
				...
				if (recipe instanceof ShapedRecipes){
					...
					//todo craftingComponents = ((ShapedRecipes)recipe).recipeItems;
					...
					//todo craftingComponents = ((ShapedOreRecipe)recipe).getInput();
				}else if (recipe instanceof ShapelessRecipes){
					//todo recipeWidth = ((ShapelessRecipes)recipe).getRecipeSize();
					recipeHeight = -1;

					craftingComponents = ((ShapelessRecipes)recipe).recipeItems.toArray();
				}else if (recipe instanceof ShapelessOreRecipe){
					//todo recipeWidth = ((ShapelessOreRecipe)recipe).getRecipeSize();
					recipeHeight = -1;

					//todo craftingComponents = ((ShapelessOreRecipe)recipe).getInput().toArray();
				}else{
					craftingComponents = null;
				}
			}else{
