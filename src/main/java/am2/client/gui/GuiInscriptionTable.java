...
//	}
	...
	private boolean drawIconSet(ArrayList<String> ids, ArrayList<String> labelText){
		...
		for (String i : ids){
			...
			//todo if (part == null || ArsMagicaAPI.getSpellRegistry().getValue(part.getRegistryName()) == null)// && SkillTreeManager.instance.isSkillDisabled(part))
				//continue;
			...
			//todo if (drawIcon(ArsMagicaAPI.getSpellRegistry().getValue(part.getRegistryName()))){
				//hovering = true;
				//labelText.add(name);
			}
	//	}
		//todo return hovering;
		return false;
	}
