	...
	public static void DrawItemAtXY(ItemStack stack, float x, float y, float zLevel, float scale){
		...
		if (scale != 1.0f){
			...
			if (stack.getItem() instanceof ItemSpellComponent) {
				Minecraft.getMinecraft().renderEngine.bindTexture(LOCATION_BLOCKS_TEXTURE);
				//todo TextureAtlasSprite icon = SpellIconManager.INSTANCE.getSprite(ArsMagicaAPI.getSpellRegistry().getValue(ArsMagicaAPI.getSkillRegistry().getObjectById(stack.getItemDamage()).getRegistryName()).getRegistryName().toString());
				GlStateManager.color(1, 1, 1, 1);
				//if (icon != null)
...
//		}
	...
	public static boolean overrideMouseInput(EntityRenderer renderer, float f, boolean b){
		...
		if (mc.gameSettings.smoothCamera){
			...
			//todo mc.player.setAngles(-f3, f4 * (float)b0);
		}else{
			//todo mc.player.setLocationAndAngles(-f3, f4 * (float)b0);
		}
