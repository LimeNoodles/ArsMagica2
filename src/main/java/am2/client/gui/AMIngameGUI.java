	...
	private void RenderArmorStatus(int i, int j, Minecraft mc, FontRenderer fontRenderer){
		...
		for (int slot = 0; slot < 4; ++slot){
			if (ArmorHelper.PlayerHasArmorInSlot(mc.player, EntityEquipmentSlot.values()[5 - slot])){
				...
				GlStateManager.color(1.0f, 1.0f, 1.0f);
				//todo ItemStack armor = mc.player.inventory.armorInventory[3 - slot];
				float lineweight = 4f;
				//durability
...
//				}else{
				GlStateManager.pushMatrix();
//todo 				AMGuiHelper.DrawItemAtXY(mc.player.inventory.armorInventory[3 - slot], position.iX, position.iY, this.zLevel, 0.63f);
				GlStateManager.popMatrix();
//				}
			}
		}
