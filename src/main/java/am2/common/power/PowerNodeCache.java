...
public class PowerNodeCache{
	...
	public void onChunkUnload(ChunkEvent.Unload event){
		if (!event.getWorld().isRemote && PowerNodeRegistry.For(event.getWorld()).hasDataForChunk(event.getChunk())){
			...
			//todo PowerNodeRegistry.For(event.getWorld()).SaveChunkToNBT(event.getChunk().getChunkCoordIntPair(), dataCompound);
			PowerNodeRegistry.For(event.getWorld()).unloadChunk(event.getChunk());
			//todo SaveNBTToFile(event.getWorld(), event.getChunk().getChunkCoordIntPair(), dataCompound, false);
		}
	}
	
	@SubscribeEvent
	public void onChunkLoad(ChunkEvent.Load event) {
		//todo NBTTagCompound dataCompound = LoadNBTFromFile(event.getWorld(), event.getChunk().getChunkCoordIntPair());
		//todo if (dataCompound != null)
			//todo PowerNodeRegistry.For(event.getWorld()).LoadChunkFromNBT(event.getChunk().getChunkCoordIntPair(), dataCompound);
	}
