...
public class CompendiumUnlockHandler{
	...
	public void onPlayerPickupItem(EntityItemPickupEvent event){
		...
		//todo instance.unlockRelatedItems(event.getItem().getEntityItem());
	}
	...
	public void onEntityDeath(LivingDeathEvent event){
		if (event.getEntityLiving().world.isRemote && event.getEntity() instanceof EntityPlayer){
			if (event.getEntity() instanceof EntityEnderman){
				ArcaneCompendium.For((EntityPlayer)event.getEntity()).unlockEntry("blockastralbarrier");
			}else{
				...
				if (reg != null && reg.getContainer().matches(ArsMagica2.instance)){
					...
					ArcaneCompendium.For((EntityPlayer)event.getEntity()).unlockEntry(id);
				}
			}
		}
	}
