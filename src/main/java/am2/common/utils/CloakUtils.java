...
public class CloakUtils{
	...
	public static void renderCloakModel(PlayerEntity player, BipedModel mainModel, float partialRenderTick){
		if (!ArsMagica2.proxy.playerTracker.hasCLDM(player.getUniqueID().toString()))
			return;
		
		if (player.isWearing(PlayerModelPart.CAPE)) return; //cloaks obey the inverse of show cape
		...
		double dx = (player.prevPosX + (player.getPosX() - player.prevPosX) * partialRenderTick) - (localPlayer.prevPosX + (localPlayer.posX - localPlayer.prevPosX) * partialRenderTick);
		double dy = (player.prevPosY + (player.getPosY() - player.prevPosY) * partialRenderTick) - (localPlayer.prevPosY + (localPlayer.posY - localPlayer.prevPosY) * partialRenderTick);
			dy += player.getHeight() - player.getYOffset() - 0.13f;
		double dz = (player.prevPosZ + (player.getPosZ() - player.prevPosZ) * partialRenderTick) - (localPlayer.prevPosZ + (localPlayer.posZ - localPlayer.prevPosZ) * partialRenderTick);
