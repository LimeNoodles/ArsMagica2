...
public class TileEntityParticleEmitter extends TileEntity implements ITickable{
	...
	public void update(){
		...
		if (!show && !world.isRemote && ((forceShow && showTicks++ > 100) || !forceShow)){
			showTicks = 0;
			forceShow = false;
			//todo for (EntityPlayer player : world.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB(pos).expandXyz(8D))) {
		...
		//}
		//todo world.markAndNotifyBlock(pos, world.getChunkFromBlockCoords(pos), world.getBlockState(pos), world.getBlockState(pos), 3);
