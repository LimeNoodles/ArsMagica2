...
public class TileEntityFlickerLure extends TileEntityAMPower{
	...
	public void update(){
		...
		if (ticksExisted % 200 == 0) {
			...
			//todo List<EntityFlicker> flickers = world.getEntitiesWithinAABB(EntityFlicker.class, new AxisAlignedBB(pos).expandXyz(5));
			/*for (EntityFlicker flicker : flickers) {
				...
				if (inventory != null) {
					...
					for (int i = 0; i < inventory.getSizeInventory(); i++) {
						...
						if (is == null || is.getItem() != ItemDefs.flickerJar || is.getItemDamage() != //ArsMagicaAPI.getAffinityRegistry().getId(Affinity.NONE))
						 continue;
						//todo is.stackSize--;
						if (is.getCount() <= 0)
							is = null;
