...
public class TileEntityCalefactor extends TileEntityAMPower implements IInventory, ISidedInventory, IKeystoneLockable<TileEntityCalefactor>, ITileEntityAMBase {
	...
	private boolean canSmelt(){
		if (this.calefactorItemStacks[0] == null){
			...
			//todo int result = calefactorItemStacks[1].stackSize + var1.stackSize;
		...
		if (this.canSmelt()){
			...
			if (doSmelt){
				...
				if (this.calefactorItemStacks[1] == null){
					...
					//todo calefactorItemStacks[1].stackSize += smeltStack.getCount();
				...
				if (Math.random() <= 0.25){
					if (calefactorItemStacks[5] == null){
						...
						//todo calefactorItemStacks[5].stackSize++;
			...
			//todo --this.calefactorItemStacks[0].get;
	...
	public void readFromNBT(NBTTagCompound nbttagcompound){
		...
		for (int i = 0; i < nbttaglist.tagCount(); i++){
			...
			if (byte0 >= 0 && byte0 < calefactorItemStacks.length){
				//todo calefactorItemStacks[byte0] = ItemStack.loadItemStackFromNBT(nbttagcompound1);
			}
		}
	}
