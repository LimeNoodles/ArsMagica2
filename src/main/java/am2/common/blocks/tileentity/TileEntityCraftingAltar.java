...
//	private static final int lectern_mutex = 4;
		...
		if (items.size() == 1){
			...
			if (item != null && !item.isDead && item.getItem() != null && item.getItem().getItem() == ItemDefs.spellParchment){
				if (!world.isRemote){
					...
					//todo world.playSound(pos.getX(), pos.getY(), pos.getZ(), "arsmagica2:misc.craftingaltar.create_spell", 1.0f, 1.0f, true);
				}
			}
		}
	}
	...
	public void readFromNBT(NBTTagCompound nbttagcompound){
		...
		if (altarCompound.hasKey("phylactery")){
			...
			if (phylactery != null)
				//todo this.addedPhylactery = ItemStack.loadItemStackFromNBT(phylactery);
				return;
		}
		...
		if (altarCompound.hasKey("catalyst")){
			...
			if (catalyst != null)
				//todo this.addedBindingCatalyst = ItemStack.loadItemStackFromNBT(catalyst);
				return;
		}
		...
		for (int i = 0; i < allAddedItems.tagCount(); ++i){
			...
			if (addedItem == null)
				continue;
			//todo ItemStack stack = ItemStack.loadItemStackFromNBT(addedItem);
			//if (stack == null)
				//continue;
