...
public class ContainerMagiciansWorkbench extends AM2Container{
	...
	public void onCraftMatrixChanged(IInventory par1iInventory){
		//todo this.workbenchInventory.firstCraftResult.setInventorySlotContents(0, CraftingManager.getInstance().findMatchingRecipe(this.firstCraftMatrix, world));
		if (!initializing){
			for (int i = 0; i < 9; ++i){
		...
		//todo this.workbenchInventory.secondCraftResult.setInventorySlotContents(0, CraftingManager.findMatchingRecipe(this.secondCraftMatrix, world));
		if (!initializing){
			for (int i = 0; i < 9; ++i){
