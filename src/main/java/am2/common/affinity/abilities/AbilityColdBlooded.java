...
public class AbilityColdBlooded extends AbstractAffinityAbility {
	...
	public void applyHurt(EntityPlayer player, LivingHurtEvent event, boolean isAttacker) {
		if (!isAttacker && event.getEntity() instanceof EntityLivingBase){
			...
			if (effect != null && event.getSource() != null && event.getEntity() != null){
				try {
					((EntityLivingBase)event.getEntity()).addPotionEffect(effect);
